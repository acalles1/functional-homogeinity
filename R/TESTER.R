...
Bootstrapper <- function(J, G, B=1000, depth.function=depthf.fd1){
  ...
  depths.inJ <- depth.function(Hf, Jf, range=c(0,1), d=30)
  depths.inG <- depth.function(Hf, Gf, range=c(0,1), d=30)
  ...
  for (i in 1:B){
    ...
    #Resampled depth, s
    depths.inJs <- depth.function(Hsf, Jfs, range=c(0,1), d=30)
    depths.inGs <- depth.function(Hsf, Gfs, range=c(0,1), d=30)
...
Tester <- function(J, G, B=1000, depth.function=depthf.fd1){
...
Bootstrapper2 <- function(J, G, B=1000, depth.function=depthf.fd1){
  # Computes the bootstrap statistics parallely.
  #
  # Args:
  # J: First sample, normally the reference sample: Sample 0.
  ...
  for (i in 1:B){
    ...
    Gfs <- rawfd2dataf(Gs, c(0,1))
    #Resampled depth, s
    depths.inJs <- depth.function(Hsf, Jfs, range=c(0,1), order=2, approx=150)
    depths.inGs <- depth.function(Hsf, Gfs, range=c(0,1), order=2, approx=150)
    depys <- depths.inJs$Half_FD
    depxs <- depths.inGs$Half_FD
    lm.bs <- lm(depys ~ depxs)
    # Betas
...
Tester2 <- function(J, G, B=1000, depth.function=depthf.fd1){
  # Tests homogeinity using bootstrap confidence intervals. H0: Homogeinity.
  #
  # Args:
  # J: First sample, normally the reference sample: Sample 0.
  ...
  Jf <- rawfd2dataf(J, c(0,1))
  Gf <- rawfd2dataf(G, c(0,1))
  depths.inJ <- depth.function(Hf, Jf, range=c(0,1), order=2, approx=150)
  depths.inG <- depth.function(Hf, Gf, range=c(0,1), order=2, approx=150)
  depy <- depths.inJ$Half_FD
  depx <- depths.inG$Half_FD
  lm.ori <- lm(depy ~ depx)
  b1.ori <- lm.ori$coefficients[2]
  b0.ori <- lm.ori$coefficients[1]
  stats <- Bootstrapper2(J, G, B=B, depth.function=depth.function)
  # Extract all statistics from the bootstrapper.
  b0 <- stats$b0
  b1 <- stats$b1
  t0 <- stats$t0
...
K <- 100
res_ord1 <- matrix(0, nrow = 6, ncol = 6)
res_ord2 <- matrix(0, nrow = 6, ncol = 6)
...
for (i in 1:6){
   for (j in i:6){
      ...
      res_ord1[i, j] <- 1 - suma1/K
      print('res1')
      print(res_ord1[i, j])
      print('res2')
      res_ord2[i, j] <- 1 - suma2/K
      print(res_ord2[i,j])
   }
}
